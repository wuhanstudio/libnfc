...
#define DBG_SECTION_NAME  "libnfc.chip.pn53x"
...
pn53x_transceive(struct nfc_device *pnd, const uint8_t *pbtTx, const size_t szTx, uint8_t *pbtRx, const size_t szRxLen, int timeout)
{
  ...
  PNCMD_TRACE(pbtTx[0]);
  if (timeout > 0) {
    LOG_D("Timeout value: %d", timeout);
  } else if (timeout == 0) {
    LOG_D("No timeout");
  } else if (timeout == -1) {
    timeout = CHIP_DATA(pnd)->timeout_command;
  } else {
    LOG_E("Invalid timeout value: %d", timeout);
  }
                                 ...
                                 const size_t szRx, int timeout)
{
  ...
  if (pbtRx != NULL) {
    if (szRxLen >  szRx) {
      LOG_E("Buffer size is too short: %" PRIuPTR " available(s), %" PRIuPTR " needed", szRx, szRxLen);
      return NFC_EOVFLOW;
    }
    // Copy the received bytes
...
pn53x_initiator_transceive_bytes_timed(struct nfc_device *pnd, const uint8_t *pbtTx, const size_t szTx, uint8_t *pbtRx, const size_t szRx, uint32_t *cycles)
{
  ...
  while (1) {
    ...
    if (pbtRx != NULL) {
      if ((szRxLen + sz) > szRx) {
        LOG_E("Buffer size is too short: %" PRIuPTR " available(s), %" PRIuPTR " needed", szRx, szRxLen + sz);
        return NFC_EOVFLOW;
      }
      // Copy the received bytes
      for (i = 0; i < sz; i++) {
  ...
  if (pnd->bCrc) {
    ...
    if ((txmode & SYMBOL_TX_FRAMING) == 0x00)
      ...
      LOG_E("Unsupported framing type %02X, cannot adjust CRC cycles", txmode & SYMBOL_TX_FRAMING);
...
{
  int ret;
  LOG_D("target_is_present(): Ping -4A");
  if (CHIP_DATA(pnd)->type == PN533) {
...
{
  int ret;
  LOG_D("target_is_present(): Ping Jewel");
...
{
  int ret;
  LOG_D("target_is_present(): Ping MFUL");
...
{
  int ret;
  LOG_D("target_is_present(): Ping MFC");
...
{
  int ret;
  LOG_D("target_is_present(): Ping DEP");
...
{
  LOG_D("target_is_present(): Ping Felica");
...
{
  int ret;
  LOG_D("target_is_present(): Ping -4B");
...
{
  int ret;
  LOG_D("target_is_present(): Ping B");
...
{
  int ret;
  LOG_D("target_is_present(): Ping B2 ST SRx");
...
{
  int ret;
  LOG_D("target_is_present(): Ping B2 ASK CTx");
...
pn53x_initiator_target_is_present(struct nfc_device *pnd, const nfc_target *pnt)
{
  ...
  if (CHIP_DATA(pnd)->current_target == NULL) {
    LOG_D("target_is_present(): no saved target");
  ...
  if ((pnt != NULL) && (!pn53x_current_target_is(pnd, pnt))) {
    LOG_D("target_is_present(): another target");
  ...
  switch (CHIP_DATA(pnd)->current_target->nm.nmt) {
    case NMT_ISO14443A:
      if (CHIP_DATA(pnd)->current_target->nti.nai.btSak & 0x20) {
        ...
        LOG_D("target_is_present(): card type A not supported");
        ret = NFC_EDEVNOTSUPP;
      }
      break;
    case NMT_DEP:
    ...
    case NMT_ISO14443B2CT:
      ...
      break;
    default:
      LOG_D("target_is_present(): card type not supported");
      ret = NFC_EDEVNOTSUPP;
      break;
  }
...
pn53x_check_error_frame(struct nfc_device *pnd, const uint8_t *pbtRxFrame, const size_t szRxFrameLen)
{
  if (szRxFrameLen >= sizeof(pn53x_error_frame)) {
    if (0 == memcmp(pbtRxFrame, pn53x_error_frame, sizeof(pn53x_error_frame))) {
      LOG_D("PN53x sent an error frame");
      pnd->last_error = NFC_EIO;
      return pnd->last_error;
    }
  }
...
pn53x_build_frame(uint8_t *pbtFrame, size_t *pszFrame, const uint8_t *pbtData, const size_t szData)
{
  if (szData <= PN53x_NORMAL_FRAME__DATA_MAX_LEN) {
    ...
    LOG_D("We can't send more than %d bytes in a raw (requested: %" PRIdPTR ")", PN53x_EXTENDED_FRAME__DATA_MAX_LEN, szData);
    return NFC_ECHIP;
  }
  return NFC_SUCCESS;
}
