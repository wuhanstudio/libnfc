...
#define DBG_SECTION_NAME  "libnfc.general"
...
nfc_device *
nfc_open(const char* uart_name)
{
  nfc_device *pnd  = uart_driver->open((void*)uart_name);
  if(pnd!=NULL){
    LOG_D("NFC device has been claimed.");
    strcpy(pnd->name, uart_name);
  }else{
    LOG_E("Failed to open NFC device");
  }
  return pnd;
}
